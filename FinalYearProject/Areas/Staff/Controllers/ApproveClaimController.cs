...
namespace FinalYearProject.Areas.Staff.Controllers
{
    ...
    public class ApproveClaimController : Controller
    {
     ...
     [Bind("claim_id, staff_id, approval_status, claim_reason, date_apply, reject_reason, claimAmount, claimFile")] EmployeeClaim claim,
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (existingClaim != null)
                    {
                        ...
                        if (existingClaim.approval_status == "Approved" && approvedAmount.HasValue)
                        {
                            
                            if (float.TryParse(existingClaim.claimAmount, out var claimAmountDecimal))
                            {
                                
                                var existingAdditionalSalary = await _db.AdditionalSalary
                                    .Where(s => s.EmployeeId == existingClaim.staff_id && s.ClaimId == existingClaim.claim_id)
                                    .FirstOrDefaultAsync();

                                if (existingAdditionalSalary != null)
                                {
                                    
                                    existingAdditionalSalary.ApprovedAmount = approvedAmount.Value;
                                    existingAdditionalSalary.DateAdded = DateTime.Now;
                                    existingAdditionalSalary.ClaimFile = existingClaim.claimFile;

                                    _db.Update(existingAdditionalSalary);
                                }
                                else
                                {
                                    
                                    var additionalSalary = new AdditionalSalary
                                    {
                                        EmployeeId = existingClaim.staff_id,
                                        ClaimId = existingClaim.claim_id,
                                        ClaimAmount = claimAmountDecimal,
                                        ApprovedAmount = approvedAmount.Value,
                                        DateAdded = DateTime.Now,
                                        ClaimFile = existingClaim.claimFile
                                    };

                                    _db.Add(additionalSalary);
                                }
                            ...
                            else
                            {
                                ...
                                ViewBag.ErrorMessage = "Invalid ClaimAmount. Please ensure it is a valid number.";
                                return View(claim);
                            }
                        }
                    }
